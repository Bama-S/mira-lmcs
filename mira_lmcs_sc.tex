...
Imperatives are not evaluated to \textit{True} or \textit{False}, which makes it difficult to use the classical logic. There has been several studies on imperative logic on different fronts such as the imperatives converted to propisitions, after which the classical logic rules are applied. But in such cases, it leads to paradox as suggested by Ross. The details of different views of imperative logic has been surveyed in the study of action representation and imperatives from the perspective of AI \cite{survey}. Inspired by the Indian Philosophical system of \mimamsa, there has been work on formalising it with the evaluation of three values namely \textit{Satisfaction} ($S$), \textit{Violation}($V$) and \textit{no intention to achieve the goal}(N) \citep{mira}. Extending this formalism the temporal sequencing of instructions has also been attempted by the same authors \citep{llmmira}.
