...
In the paper “Goal and Plan Recognition Design for Plan Libraries,” the authors extend the traditional Goal Recognition Design (GRD), originally defined for STRIPS domains, to one that incorporates plan libraries, enabling the representation of hierarchical structures and complex agent behaviors \citep{plan_library}. They define two related problems: Goal Recognition Design for Plan Libraries (GRD-PL), which focuses on identifying what goal an agent is pursuing, and Plan Recognition Design (PRD), which focuses on how the goal is achieved. Two key metrics are introduced: Worst-Case Distinctiveness (wcd), measuring the maximum number of observations needed to unambiguously determine the goal, and Worst-Case Plan Distinctiveness (wcpd), for determining the specific plan. The study demonstrates that wcd serves as a theoretical lower bound for wcpd, offering domain designers practical tools to quantify and minimize ambiguity, thereby enhancing the accuracy and efficiency of real-time plan and goal recognition systems.

\subsection*{Dependency and Semantic Relations}

While traditional planning frameworks emphasize causal or temporal linkages between actions, our work instead focuses on uncovering \emph{object dependencies} that bind a set of interrelated instructions or statements. Such dependencies determine when the fulfilment or applicability of one instruction relies upon the existence, modification, or outcome of another through shared objects. This form of intra-sequential dependency has parallels in both linguistic and cognitive models of semantic representation.

The earliest formal groundwork in this direction is found in \emph{Conceptual Dependency Theory} \citep{patterson_2004}, which sought to represent meaning through primitive acts and their interconnections with actors and objects. Each sentence was converted into a dependency structure that preserved semantic consistency independent of syntactic form. This model provided one of the first approaches to capturing inter-statement meaning equivalence and referential dependency.

Subsequent computational semantics research has extended these ideas through \emph{dependency-based semantic analysis} \citep{johansson_nugues_2008} and \emph{semantic role labeling} frameworks \citep{aher_kritika_2019}. These methods model how predicates (typically verbs) and their corresponding arguments (objects or entities) interact to form coherent action--object networks. Such representations align closely with our formal abstraction of instruction dependencies, where each instruction is treated as an action--object pair, and dependencies express transmission of interpretational relevance across those pairs.

A particularly relevant extension is found in the work by \citet{makni_2012}, which formalizes semantic relations between actions in business processes using shared objects as the basis for equivalence, inclusion, and independence. Their approach explicitly codifies when two activities refer to a common object or yield one another through transformation—providing a directly analogous model for our object-dependency condition within instruction sequences.

Complementary insights are drawn from cognitive studies of sequential dependencies, which show that structured symbolic reasoning often arises from the ability to detect long-range associations between symbolic units \cite{sainburg_2022, goffman_2023}. Similarly, in artificial intelligence reasoning contexts, such dependencies manifest as relationships among involved entities and their resultant states \cite{kartmann_2021, yin_2025}.
