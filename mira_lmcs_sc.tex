...
Assume a robot is assigned the task of \textit{cooking tomato noodles stir-fry}. While this may seem straightforward at first, several intricate considerations must be addressed before executing each instruction. For example, this task may involve multiple steps such as \textit{``pick noodles, cook noodles in pot, chop tomato, fry tomato, and add the cooked noodles to a dish”}.

Consider another task in which a robot must paint 10 fence panels, each requiring three steps: applying primer, then the first coat, and finally the second coat of paint. This task can be executed in two different ways. In one approach, the robot completes all three steps on a single panel before proceeding to the next. Alternatively, the robot can perform each step across all panels in sequence: first applying primer to every panel, then the first coat, followed by the second coat.

These examples highlight several challenges in task execution. The robot must determine which actions to perform, which objects to use, and the optimal order of actions to maximize efficiency.

Over the years, numerous studies have explored reasoning with actions across different domains, including hierarchical planning, temporal logic, Belief-Desire-Intention (BDI) models, Natural Language Processing, and Large Language Models. Despite this progress, effectively sequencing instructions for successful task execution remains a significant challenge.

This paper addresses this challenge by formalizing various instruction sequencing methods inspired by the Indian philosophical system of \mimamsa. These methods include: direct assertion sequencing (\textit{'Srutikrama}), purpose-driven sequencing (\textit{Arthakrama}), and step-by-step parallel sequencing (\textit{Pravṛttikrama}). Building upon the MIRA framework \citep{mira} and temporal reasoning approaches \citep{llmmira}, instructions are treated as action–object pairs, which serve as the fundamental unit for sequencing in these three methods.
